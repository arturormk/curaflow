...
from ...fetcher import FetchMeta, conditional_get, SRC_DIR
from ...utils import ensure_dir, write_text_atomic, sha256_obj
from ...html_utils import make_soup, slugify

async def fetch(name: str, params: Dict[str, Any]) -> Tuple[bool, Dict[str, Any], List[Dict[str, Any]]]:
    ...
    """
    url = params["url"]; headers = params.get("headers") or {}
    ...
    for es in extract_specs:
        ename = es["name"]; css = es["css"]; attr = es.get("attr", "text")
        base = es.get("base") or url
        items: List[Dict[str, Any]] = []
        for el in soup.select(css):
            rec: Dict[str, Any] = {}
            if attr == "text":
                value = el.get_text(strip=True); rec["text"] = value
            else:
                value = el.get(attr) or ""; rec[attr] = value
    ...
    if meta and meta.digest == digest:
        new_meta = FetchMeta(name=name, url=url, etag=resp.headers.get("ETag"), last_modified=resp.headers.get("Last-Modified"), digest=digest)
        new_meta.save()
        prev = yaml.safe_load((SRC_DIR / f"{name}.yaml").read_text(encoding="utf-8")) if (SRC_DIR / f"{name}.yaml").exists() else None
        return (False, prev, [])

    write_text_atomic(SRC_DIR / f"{name}.yaml", yaml.safe_dump(normalized, sort_keys=True, allow_unicode=True))
    new_meta = FetchMeta(name=name, url=url, etag=resp.headers.get("ETag"), last_modified=resp.headers.get("Last-Modified"), digest=digest)
    new_meta.save()
    ...
    for fs in fanout_specs:
        src = fs["from"]; child_plugin = fs["plugin"]
        ...
        for idx, item in enumerate(ex.get(src, [])):
            if url_field not in item: continue
            child_name = name_t.format(slug=item.get("slug", f"{idx}"), index=idx, absolute_url=item.get("absolute_url", ""))
